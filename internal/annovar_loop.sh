...
if [[ -n "$ITHE_MODULE_ANNOVAR" ]]
then
    if [[ "$ITHE_MOD_ISA" -eq 1 ]]
    then
        if [[ $(module is-avail "$ITHE_MODULE_ANNOVAR") -eq 1 ]]
        then
            module is-loaded "$ITHE_MODULE_ANNOVAR" || module load "$ITHE_MODULE_ANNOVAR"
        else
            echo "ERROR: The module $ITHE_MODULE_ANNOVAR is not available"
            exit 1
        fi
    else
        module load "$ITHE_MODULE_ANNOVAR"
    fi 
fi

if [[ -n "$ITHE_EXE_ANNOVAR" ]]
then
    eval "$ITHE_EXE_ANNOVAR"
fi


if [[ -z "$ITHE_HUMANDB_DIR" ]]
then
    echo "ERROR: The environment variable ITHE_HUMANDB_DIR needs to point to the human genome for annovar to work\n";
    exit 1
else
    if [[ $(ls $ITHE_HUMANDB_DIR/*.fa | wc -l) -ne 1 ]]
    then
        echo "ERROR: The environment variable ITHE_HUMANDB_DIR is not properly set, there is no *.fa genome file there\n"
        exit 1
    fi
fi
...
annotate ()
{
    ...
    then
        ...
        annotate_variation.pl --geneanno --buildver hg19 --outfile ${outputfile}.annotated ${outputfile}.inputann $ITHE_HUMANDB_DIR
