...
usage="\n$0 directory torun_file exe_params filtering_params NAB_params covB_params popAF_params n_cores tstv\n\ntorun_file structure: output N_file A_file B_file\n-------------------------------------------------\n
\n
This script postprocess the output of ITHE_control.pl for each sample in a directory with its name, integrating all the information in a file named results.csv. Results results_basictstv.csv is generated only if tstv==1.\n"
...
do
    ...
    if [[ $tstv -eq 1 ]]
    then
        $ITHE_INT/perl.sh $ITHE_INT/tstv_pretabulate_heter.pl $i/tstv.csv $i/tstv.tomerge
        $ITHE_INT/perl.sh $ITHE_INT/merge_csv.pl $i/csvtomerge.temp $i/tstv.tomerge $i/${output}_withtstv.temp
        rm -f $i/csvtomerge.temp
        $ITHE_INT/perl.sh $ITHE_INT/tabulate_results.pl -i $i/${output}_withtstv.temp -o $i/${output}_withtstv_tab.temp
        ...
        $ITHE_INT/perl.sh $ITHE_INT/tabulate_results.pl -i $i/csvtomerge.temp -o $i/csvtomerge_tab.temp
