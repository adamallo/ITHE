...
#Workload manager
my $queue="";
my $queue_argument=$ENV{'ITHE_SUBMIT_PAR'};
my $ithe_time=$ENV{'ITHE_MAX_TIME'};
my $n_cores=1;
my $scheduler=$ENV{'ITHE_SUBMIT_CMD'};
my $qsub=$ENV{'ITHE_SUBMIT_MUL'};
my $qsub_noparallel=''; 
...
if ( ! -f $sample1_bam)
{
	die "The BAM file $sample1_bam is not accesible. Please, check input options.\n";
}
else
{
...
if ( ! -f $sample2_bam)
{
	die "The BAM file $sample2_bam is not accesible. Please, check input options.\n";
}
else
{
...
if (-f "$ITHE_INT/$variant_caller.sh")
{
    $variant_calling_sh="$ITHE_INT/$variant_caller.sh";
}
else
{
    die "Error, the sh file for the variant caller $variant_caller can't be located. Please, make sure that the environment variable ITHE_INT is indicating the folder with this package of scripts\n"
}

if (-f "$ITHE_INT/$helper_sh" && -f "$ITHE_INT/$helper_pl" && -f "$ITHE_INT/$reduce_sh" && -f "$ITHE_INT/$covB_sh" && -f "$ITHE_INT/$covN_sh")
{
    $helper_sh="$ITHE_INT/$helper_sh";
    $helper_pl="$ITHE_INT/$helper_pl";
    $reduce_sh="$ITHE_INT/$reduce_sh";
    $covB_sh="$ITHE_INT/$covB_sh";
    $covN_sh="$ITHE_INT/$covN_sh";
}
else
{
    die "Error, the files for the secondary analysis of the data, $helper_sh, $helper_pl, $reduce_sh, $covB_sh, and $covN_sh can't be located. Please, make sure that the environment variable ITHE_INT is indicating the folder with this package of scripts\n"
}

if (-f "$ITHE_INT/$tstv_sh")
{
    $tstv_sh="$ITHE_INT/$tstv_sh";
}
else
{
  die "Error, the sh file to get the TsTv data, $tstv_sh can't be located. Please, make sure that the environment variable ITHE_INT is indicating the folder with this package of scripts\n"
}

if (-f "$ITHE_INT/$annotation_sh")
{
    $annotation_sh="$ITHE_INT/$annotation_sh";
}
else
{
  die "Error, the sh file to annotate variants using annovar, $annotation_sh can't be located. Please, make sure that the environment variable ITHE_INT is indicating the folder with this package of scripts\n"
}
...
my %exe_deps; ##Dependencies of a specific group of samples with a specific exe_condition --> For ITHE (includes covN)
my $deps;
my $tempofile;
my $tag;
my $EXETOFILE;
...
foreach my $exe_condition (@exe_conditions)
{
    ...
    if(-f $onfile2)
    { 
        $job_id=submit_job("$qsub $deps $ithe_time -J ITHE.$samplename.$tag $helper_sh $helper_pl -e exeparams.$tag -f $offile --NABfilt_cond_inputfile $onfile --NABfilt_cond_inputfile2 $onfile2 --covaltB_cond_inputfile $ocfile --popAF_cond_inputfile $opfile -o $tempofile --output_vcf $output_vcf --output_list $output_list --comp $comp --n_cores $n_cores");
    }
    else
    {
        $job_id=submit_job("$qsub $deps $ithe_time -J ITHE.$samplename.$tag $helper_sh $helper_pl -e exeparams.$tag -f $offile --NABfilt_cond_inputfile $onfile --covaltB_cond_inputfile $ocfile --popAF_cond_inputfile $opfile -o $tempofile --output_vcf $output_vcf --output_list $output_list --comp $comp --n_cores $n_cores");
    }
